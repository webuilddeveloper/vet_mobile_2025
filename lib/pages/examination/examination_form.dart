...
import 'package:vet/pages/examination/examination_dialog.dart';
import 'package:vet/pages/examination/examination_list.dart';
...
class ExaminationForm extends StatefulWidget {
  ...
  @override
  _ExaminationDetailPageState createState() => _ExaminationDetailPageState();
}

class _ExaminationDetailPageState extends State<ExaminationForm> {
  ...
  void initState() {
    ...
    // sharedApi();
    // getUserData();
    _futureModel = postDio('${examinationApi}all/read', {
      'skip': 0,
      'limit': 1,
      'code': widget.code,
    });
            ...
            builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
              ...
              if (snapshot.hasData) {
                List.generate(
                  ...
                  (i) => ooo.add(<TextEditingController>[]),
                );
  ...
  checkBoxMultiple(dynamic answers, int i) {
    return Container(
      ...
      child: CheckboxListTile(
        ...
        onChanged: (bool? value) {
          setState(() {
            answers[i]['value'] = !(answers[i]['value'] ?? false);
          });
        },
  ...
  examinationReply(dynamic model) async {
    ...
    if (checkSendReply) {
              ...
              question['answers']
                  .map(
                    (answer) => {
                      if (answer['value'])
                        {
                          postObjectData('m/examination/reply/create', {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (context) => ExaminationDialog(titleHome: widget.titleHome),
        ),
      );
      // showDialog(
      //   barrierDismissible: false,
      //   context: context,
      //   builder: (BuildContext context) => new ExaminationDialog(userData: userData,titleHome: widget.titleHome,),
      // );
    }
  }
  ...
  void goBack() async {
    ...
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(
        builder: (context) => ExaminationList(title: widget.titleHome),
      ),
    );
  }
}
